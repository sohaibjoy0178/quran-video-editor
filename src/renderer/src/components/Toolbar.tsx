...
import { renderVideoWebCodecs } from '../utils/webCodecsRender'
...
export function Toolbar({ onOpenSettings }: ToolbarProps): React.ReactNode {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const rawCaptions = Array.isArray(result) ? (result as any) : result.captions
      ...
      const captionsWithIds = rawCaptions.map(
        (c: { start: number; end: number; arabic: string; bengali: string }, i: number) => ({
          ...c,
          id: `cap-${i}`,
          arabicLines: [c.arabic], // Default to single line, will be wrapped in renderer
          bengaliLines: [c.bengali]
        })
      )
  ...
  const handleRender = useCallback(async () => {
    ...
    if (!videoPath || !effectiveAudioPath || captions.length === 0) {
      console.warn('Render blocked: missing paths or captions', {
        videoPath,
        audioPath,
        captionsLen: captions.length
      })
      return
    }
    ...
    try {
      const blob = await renderVideoWebCodecs({
        videoPath,
        ...
        appendDiagnostic: appendRenderDiagnostic
      })

      setGeminiStatus('Saving file...')
      const buffer = await blob.arrayBuffer()
      const result = await window.api.files.write(outputPath, buffer)

      if (result.success) {
        setGeminiStatus('Render Complete!')
      ...
      setGeminiStatus(`Render Error: ${msg}`)

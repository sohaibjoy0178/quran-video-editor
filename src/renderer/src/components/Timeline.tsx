...
function CaptionBlock({ cap, pxPerSec, totalDuration, variant, onDragStart, onDragEnd }: CaptionBlockProps) {
    ...
    const handleMouseDown = useCallback(
        (e: React.MouseEvent, type: 'left' | 'right' | 'move') => {
            ...
            document.body.style.cursor =
                type === 'left' || type === 'right' ? 'col-resize' : 'grabbing'
            document.body.style.userSelect = 'none'
            document.addEventListener('mousemove', onMove)
            document.addEventListener('mouseup', onUp)
        },
        [cap.id, cap.start, cap.end, pxPerSec, totalDuration, updateCaption, setCurrentTime, onDragStart, onDragEnd]
    )
